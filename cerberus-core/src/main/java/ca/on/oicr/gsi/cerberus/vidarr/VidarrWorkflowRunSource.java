...
public final class VidarrWorkflowRunSource
  ...
  public UpdateResult<ProvenanceWorkflowRun<ExternalKey>> update() throws Exception {
    try (final var ignored = REQUEST_TIME.start(baseUrl)) {
      ...
      ArrayList<ProvenanceWorkflowRun<ExternalKey>> validBodyResults = new ArrayList<>();
      body.getResults().stream()
          ...
          .peek(workflowRun -> workflowRun.setInstanceName(instanceName))
          .forEach(validBodyResults::add);
