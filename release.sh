...
Cerberus uses semantic versioning. Valid release types: major, minor, patch"
    exit 3
    ;;
esac
...
git commit -a -m "Cerberus ${RELEASE_VERSION} release"
git tag -a v${RELEASE_VERSION} -m "Cerberus ${RELEASE_VERSION} release"
mvn clean install
mvn deploy
...
echo CERBERUS_VERSION=${RELEASE_VERSION}
